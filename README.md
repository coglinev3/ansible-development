...
The `Vagrantfile` contains a variable called boxes:
...
The Vagrant timezone configuration doesn't work correctly.  Thæt's why I use the solution from [Frédéric Henri](https://stackoverflow.com/users/4296747/fr%c3%a9d%c3%a9ric-henri), see: https://stackoverflow.com/questions/33939834/how-to-correct-system-clock-in-vagrant-automatically
...
If this happens, you must log in to the virtual machine as user vagrant using the VirtualBox GUI. Install the ifupdown package and switch off the maschine.
...
Sometimes it makes sense to safe the current state before testing a new Ansible Role. You can save the virtual machine state at any time, for example:
...
If you want to use ŧhe [Ansible Vault](http://docs.ansible.com/ansible/2.4/vault.html) feature you have to create a Ansible vault password file `/vagrant/provisioning/.ansible_vault` and add the following line in *vagrant's* bashrc file:
